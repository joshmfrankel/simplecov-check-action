  ...
  context "for coverage type line" do
    ...
    context "for Pull Requests" do
      it "Fails when coverage is lower than minimum" do
        ...
        stub_request(:patch, "https://api.github.com/repos/#{repo}/check-runs/#{check_run_id}")
          .with(body: { name: "SimpleCov", head_sha: pull_request_json_sha, status: "completed", completed_at: the_time, conclusion: "failure", output: { title: "97.77% covered (minimum #{minimum_coverage}%)", summary: "* 97.77% covered\n" + "* #{minimum_coverage}% minimum (by line)\n", text: passing_markdown_text, annotations: [] } })
      ...
      it "Passes when coverage is greater than or equal to minimum" do
        ...
        stub_request(:patch, "https://api.github.com/repos/#{repo}/check-runs/#{check_run_id}")
          .with(body: { name: "SimpleCov", head_sha: pull_request_json_sha, status: "completed", completed_at: the_time, conclusion: "success", output: { title: "97.77% covered (minimum #{minimum_coverage}%)", summary: "* 97.77% covered\n" + "* #{minimum_coverage}% minimum (by line)\n", text: passing_markdown_text, annotations: [] } })
